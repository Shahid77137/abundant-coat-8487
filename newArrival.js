...
asc.addEventListener("click",sortingLth);

function sortingLth(){
    console.log(true)
   let sortedbag=  bag.sort((a,b)=> {

        let val1 = +(a.items_price.replace("₹",""));

        let val2 = +(b.items_price.replace("₹",""));
        
         return val1-val2;
})
appendData(sortedbag)
}
let dsc = document.getElementById("htl");
dsc.addEventListener("onclick",sortingDth)
